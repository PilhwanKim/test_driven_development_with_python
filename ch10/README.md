...
Fabric을 사용하면 Python 스크립트 내부의 서버에서 명령을 실행할 수 있다. 이것은 서버 관리 작업을 자동화하는 훌륭한 도구이다.

#### Idempotency(멱등)

배포 스크립트가 기존 서버에 배포하는 경우 새로 설치하거나 이미 구성된 서버에 대해 작동하도록 설계해야 한다.

#### cofig 파일을 소스컨트롤에 관리

설정 파일의 유일한 사본이 서버에 없는지 확인하자.

그것들은 응용 프로그램에 중요하며 다른 것과 마찬가지로 버전 제어를 받아야한다.

#### 프로비저닝 자동화

궁극적으로 모든 것이 자동화되어야하며 여기에는 새로운 서버의 가동과 올바른 소프트웨어 설치가 포함된다. 

호스팅 제공 업체의 API와 상호 작용해야한다.

#### configuration 관리 도구

패브릭은 매우 유연하지만 로직은 여전히 ​​스크립팅을 기반으로 한다.

더 진보 된 도구는보다 `선언적인`접근 방식을 취하고 더욱 편하게 만들 수 있다.

Ansible과 Vagrant를 예로 들수 있습니다. 그외(Chef, Puppet, Salt, Juju…)도 있음.
